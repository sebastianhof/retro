...
function MaxCube(ip, port) {
    ...
    //ruleHeartbeat.second = [new schedule.Range(0, 59, heartbeatIntervalSecs)];
    //this.heartbeatJob = schedule.scheduleJob(ruleHeartbeat, function () {
    //    log('Heartbeat');
    //    doHeartbeat.call(self);
    //});
...
MaxCube.prototype.doHeartbeat = function () {
    doHeartbeat.call(this);
};

function decodeDevice(payload) {
    var rf_address = payload.slice(1, 4).toString('hex');
    if (this.devices[rf_address] != null) {
        switch (this.devices[rf_address].devicetype) {
            ...
            case EQ3MAX_DEV_TYPE_SHUTTER_CONTACT:
                return decodeDeviceShutterContact.call(this, payload);
                break;
            default:
...
function decodeDeviceThermostat(payload:any) {
    /*
    ...
    if (mode === 'VACATION') {
        ...
        deviceStatus.date_until = 2000 + (payload[10] & 0x3F) + "-" + padLeft(((payload[9] & 0xE0) >> 4) | (payload[10] >> 7), 2, null) + "-" + padLeft(payload[9] & 0x1F, 2, null);
...
function decodeDeviceShutterContact(payload:any) {
